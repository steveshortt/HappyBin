...
namespace HappyBin.AutoUpdater
{
	public class Updater : INotifyPropertyChanged
	{
		...
		public PatchStatus InitializePatchStatus()
		{
			...
			RuntimeExeInfo rei = new RuntimeExeInfo( HappyBinCli.Properties.Settings.Default.RuntimeExe );
			result.ExeInfo = rei;
			...
			if( uc.CurrentVer > rei.Version )
			{
				...
				string downloadFolderRoot = HappyBinCli.Properties.Settings.Default.DownloadFolder;
				...
				result.PatchIsValid = File.Exists( patchFilePath );
				if( !result.PatchIsValid )
				{
					result.PatchIsValid = this.GetPatchFile( patchUri, patchFilePath );
				}
			}
		...
		{
			Uri configUri = new Uri( HappyBinCli.Properties.Settings.Default.UpdateConfigUri );
		...
		{
			int waitMilliseconds = HappyBinCli.Properties.Settings.Default.WaitForExitMillseconds;
			...
			DirectoryInfo downloadFolder = new DirectoryInfo( HappyBinCli.Properties.Settings.Default.DownloadFolder );
			...
			if( HappyBinCli.Properties.Settings.Default.StartRuntimeExeAfterInstall )
			{
